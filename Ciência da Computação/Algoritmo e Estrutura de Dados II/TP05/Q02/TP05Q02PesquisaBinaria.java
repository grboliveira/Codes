  	...
  	public static void main(String[] args) throws Exception {
        ...
        int comparacao = 0;
  		 ...
  		 for(String entrada = MyIO.readLine(); entrada.equals("0") == false; entrada = MyIO.readLine()) {
            ...
            comparacao += lista.inserirFim(municipio);          
  		}

        long inicio = System.currentTimeMillis();
        comparacao += lista.insercao();

        for(String entrada = MyIO.readLine(); entrada.equals("FIM") == false; entrada = MyIO.readLine()) {
            boolean condicao = lista.pesquisar(Integer.parseInt(entrada));
            lista.mostrar(condicao);  
        }
        long fim = System.currentTimeMillis();
    
        double tempo = ((fim - inicio)/1000.0);

        Arq.openWrite("559855_binaria.txt");
...
class Lista {
    ...
    */
    ...
    /**
     * Ordena o array de pessoas pelo método inserção.
     * @param esq int primeira posição do array.
     * @param dir int última posição do array.
     */
    public int insercao() {
    	int comparacao = 0;
      	for (int i = 1; i < n; i++) {
			...
			int j = i - 1;
			comparacao++;
			while ((j >= 0) && (array[j].getId() > tmp.getId())) {
				comparacao += 2;
				array[j + 1] = array[j];
				j--;
			}
     ...
     * @param num int elemento a ser pesquisado.
     */
    public boolean pesquisar(int num) {   
    	...
    	int fim = this.n - 1; 
  	 	...
  	 	while (inicio <= fim) {                          
      		meio = (inicio + fim) / 2;                        
      		if (array[meio].getId() == num) {
      			return true;
      		} else if (array[meio].getId() < num) {
      			inicio = meio + 1;
      		} else {
      			fim = meio - 1;
      		} 
    ...
    /**
     * Mostra os elementos da lista separados por linhas.
     */
    public void mostrar (boolean resultado) {
        if(resultado == true) {
            MyIO.println("SIM");
        } else {
            MyIO.println("NÃO");
