/**
 * TP05Q09 - Ordenação Counting Sort
...
class TP05Q09OrdenacaoCountingSort {
  	public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("559855_countingsort.txt");
...
class Lista {
     ...
     */
    public int getMaior() {
        int maior = array[0].getNumeroFuncionarios();
 
        for (int i = 0; i < n; i++) {
            if(maior < array[i].getNumeroFuncionarios()) {
                comparacao++;
                maior = array[i].getNumeroFuncionarios();
            }
        }
        return maior;    
    }
    ...
    public void countingsort() {
        ...
        for(int i = 0; i < n; count[array[i].getNumeroFuncionarios()]++, i++);
 
        //Agora, o count[i] contem o numero de elemento menores ou iguais a i
        for(int i = 1; i < count.length; count[i] += count[i-1], i++);
 
        //Ordenando
        for(int i = n - 1; i >= 0; ordenado[count[array[i].getNumeroFuncionarios()] - 1] = array[i], count[array[i].getNumeroFuncionarios()]--, i--);
        movimentacao++;

        //Ordenando por id municipios com mesmo número de funcionários
        for(int i = 0; i < n - 1; i++) {
            for(int j = 0; j < n - 1; j++) {
                if(ordenado[i].getNumeroFuncionarios() == ordenado[j].getNumeroFuncionarios() && ordenado[i].getId() < ordenado[j].getId()) {
                    Municipio tmp = ordenado[j];
