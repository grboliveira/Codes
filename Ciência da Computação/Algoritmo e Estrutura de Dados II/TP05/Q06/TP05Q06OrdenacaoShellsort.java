...
class TP05Q06OrdenacaoShellsort {
  	public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("559855_shellsort.txt");
...
class Lista {
    ...
    public void insercaoPorCor(int cor, int h) {
        for (int i = (h + cor); i < n; i += h) {
            ...
            int j = i - h;
            while ((j >= 0) && (array[j].getUf().compareTo(tmp.getUf()) > 0)) {
                comparacao += 2;
                array[j + h] = array[j];
                movimentacao++;
                j -= h;
     ...
     * @param int h passo do shelsort
     */
    public void insercaoPorCorId(int cor, int h) {
        for (int i = (h + cor); i < n; i += h) {
            Municipio tmp = array[i];
            int j = i - h;
            while ((j >= 0) && (array[j].getUf().compareTo(tmp.getUf()) == 0) && (array[j].getNome().compareTo(tmp.getNome()) > 0)) {
                comparacao += 3;
                array[j + h] = array[j];
                movimentacao++;
                j -= h;
