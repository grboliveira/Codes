...
class Municipio {
  ...
  private Boolean consorcio;
     ...
     */
  	...
  	public int getDoidaoValue() {
		return getAtualizacaoPlano()*1000 + getId();
	}
...
class TP05Q12OrdenacaoRadixsort {
  	public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("559855_radixsort.txt");
...
class Lista {
	...
	public void radixsort() {
		for (int place = 1; place <= 1000000000; place *= 10) {
			radixCountingSort(place);
		}
	}

	public void radixCountingSort(int place) {
		...
		for(int i = 0; i < n; i++) {
			comparacao++;
            int digit = getDigit(array[i].getDoidaoValue(), place);
            count[digit] += 1;
        }
        ...
        for(int i = n - 1; i >= 0; i--) {
        	comparacao++;
            int digit = getDigit(array[i].getDoidaoValue(), place);
            municipio[count[digit] - 1] = array[i];
            movimentacao++;
            count[digit]--;
        }
        array = municipio;
