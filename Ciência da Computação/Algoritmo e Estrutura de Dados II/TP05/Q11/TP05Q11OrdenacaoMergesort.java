...
class TP05Q11OrdenacaoMergesort {
  	public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("559855_mergesort.txt");
...
class Lista {
    ...
    public void sort(int fim) {  
        for (int i = 0; i < fim; i++) {
        	comparacao++;
	   		for(int j = i + 1; j< fim; j++) {
                if(array[i].getNumeroComissados() > array[j].getNumeroComissados()) {
                	comparacao++;
                    swap(i,j);
                }
	   		}
     ...
     * Algoritmo de ordenacao Mergesort.
     */
   	public  void mergesortId(int fim) {
        if(fim <= 1) {
            return;
        }
    ...
    public void sortId(int fim) {  
        for (int i = 0; i < fim; i++) {
        	comparacao++;
	   		for(int j = i + 1; j< fim; j++) {
                if(array[i].getNumeroComissados() == array[j].getNumeroComissados() && array[i].getId() > array[j].getId()) {
                	comparacao += 2;
                    swap(i,j);
                }
	   		}
