...
class TP05Q08OrdenacaoQuicksort {
  	public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("559855_quicksort.txt");
...
class Lista {
    ...
    * @param int dir fim do array a ser ordenado
    ...
    public void quicksort(int esq, int dir) {
        int i = esq, j = dir;
        Municipio pivo = array[(dir+esq)/2];
        while (i <= j) {
            comparacao++;
            while (array[i].getPopulacao() < pivo.getPopulacao()) i++; comparacao++;
            while (array[j].getPopulacao() > pivo.getPopulacao()) j--; comparacao++;
            if (i <= j) {
                comparacao++;
                swap(i, j);
                i++;
