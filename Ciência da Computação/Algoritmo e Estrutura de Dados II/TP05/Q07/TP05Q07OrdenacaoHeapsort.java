...
class TP05Q07OrdenacaoHeapsort {
  	public static void main(String[] args) throws Exception {
        ...
        Arq.openWrite("559855_heapsort.txt");
...
class Lista {
     ...
     */
    public int inserirFim(Municipio municipio) throws Exception {
      int comparaçao = 0;
      //Validar inserção
      if(n >= array.length) {
    ...
    public void heapsort() throws Exception {
        ...
        Municipio [] tmp = new Municipio[n + 1];
        for(int i = 0; i < n; i++) {
            tmp[i + 1] = array[i];
        }
        array = tmp;
        ...
        tmp = array;
        array = new Municipio[n];
        for(int i = 0; i < n; i++) {
            array[i] = tmp[i+1];
        }
    }

    public void constroi(int tamHeap) throws Exception {
        for(int i = tamHeap; i > 1 && array[i].getCodigoUf() > array[i/2].getCodigoUf(); i /= 2){
            comparacao++;
            swap(i, i/2);
        }
    } 
 
    public void reconstroi(int tamHeap) throws Exception {
        int i = 1, filho;

        while(i <= (tamHeap/2)) {
            if (array[2 * i].getCodigoUf() > array[2 * i + 1].getCodigoUf() || 2 * i == tamHeap) {
                comparacao++;
                filho = 2 * i;
            } else {
                filho = 2 * i + 1;
            ...
            if(array[i].getCodigoUf() < array[filho].getCodigoUf()) {
                comparacao++;
                swap(i, filho);
                i = filho;
            } else {
    ...
    public void heapsortId() {
        //Ordenando por id municipios com mesmo número código de estado
        for(int i = 0; i < n; i++) {
            comparacao++;
            for(int j = 0; j < n - 1; j++) {
                comparacao++;
                if(array[i].getCodigoUf() == array[j].getCodigoUf() && array[i].getId() < array[j].getId()) {
                    comparacao += 2;
                    Municipio tmp = array[j];
                    array[j] = array[i];
                    array[i] = tmp;
                    movimentacao += 2;
                }
