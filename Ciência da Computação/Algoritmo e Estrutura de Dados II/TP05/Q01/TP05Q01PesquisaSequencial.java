...
class TP05Q01PesquisaSequencial {
  	public static void main(String[] args) throws Exception {
        ...
        for(String entrada = MyIO.readLine(); entrada.equals("FIM") == false; entrada = MyIO.readLine()) {
            comparacao += lista.pesquisar(Integer.parseInt(entrada));   
        }
        long fim = System.currentTimeMillis();
    
        double tempo = ((fim - inicio)/1000.0);

        Arq.openWrite("559855_sequencial.txt");
        Arq.println("Matrícula: 559855\t"+"Comparação: "+comparacao+"\t"+"Tempo: "+tempo);
        Arq.close();     
  	}
}
...
class Lista {
    ...
    */
    ...
    public int pesquisar(int num) {
        ...
        for (int i = 0; i < n && condicao == false; i++) {
            condicao = (array[i].getId() == num);
            comparacao++;
        }
        mostrar(condicao);
        return comparacao;
